...
var app = express();
var url = 'mongodb://admin:admin@ds147510.mlab.com:47510/raj';
...
app.get('/get', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection('raj').find().toArray(function(err, result){
            if(err)
            {
...
app.get('/update/:s_id', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection('raj').update(find,{'$set':newData},
            function(err,result){
                if(err)
                    throw err;
                else
...
app.get('/delete/:s_id', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection('raj').remove(find,function(err,result){
            if(err)
                throw err;
...
var insertDocument = function(db, data, callback) {
    db.collection('raj').insertOne( data, function(err, result) {
        if(err)
        {
